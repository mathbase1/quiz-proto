// questionBank.js
// Question bank + generators (N7–N9) for the prototype quiz maker.
// Loaded by index.html (GitHub Pages compatible).
// Source: gcse_number_foundation_split_03_N7-N9_UPDATED_v13_FINAL_N7-N9_randomised_items_noZero_v4.html (extracted buildQuestion + helpers)
...
/* -------------------- part constructors -------------------- */
// Infer sensible unit placeholders (matches the worksheet bank behaviour).
function __stripHtmlForUnit(s){
  return String(s||"")
    .replace(/<[^>]*>/g," ")
    .replace(/&nbsp;/g," ")
    .replace(/\s+/g," ")
...
function __lastMatchIndex(str, re){
...
function __inferUnitPlaceholder(partTextHtml){
  const t = __stripHtmlForUnit(partTextHtml);
  if(!t) return "";
  ...
  for(const c of candidates){
    const idx = __lastMatchIndex(t, c.re);
    if(idx < 0) continue;
...
function __defaultUnitPlaceholder(partTextHtml){
  const unit = __inferUnitPlaceholder(partTextHtml);
  const s = String(unit||"").trim();
  return s || "answer";
}

function partNumber(id, textHtml, marks, value, opts={}){
  const ph = (opts && typeof opts.placeholder === "string" && opts.placeholder.trim())
    ? opts.placeholder.trim()
    : __defaultUnitPlaceholder(textHtml);
  return {marks, textHtml, input:{kind:"number", id, placeholder:ph}, answer:{type:"number", value}};
}

function partInteger(id, textHtml, marks, value, opts={}){
  const ph = (opts && typeof opts.placeholder === "string" && opts.placeholder.trim())
    ? opts.placeholder.trim()
    : __defaultUnitPlaceholder(textHtml);
  return {marks, textHtml, input:{kind:"integer", id, placeholder:ph}, answer:{type:"number", value}};
}
...
function partMoney(id, textHtml, marks, value){
  return {marks, textHtml, input:{kind:"money", id, placeholder:"£"}, answer:{type:"number", value}};
}
...
if(scenario===2){
  ...
  // Expose a minimal, stable API for the dashboard.
  const __topicNameMap = {};
  try {
    if (typeof TOPICS !== "undefined" && Array.isArray(TOPICS)) {
      for (const t of TOPICS) __topicNameMap[t.code] = t.name;
    }
  } catch(e) {}

  function __sumMarks(parts){
    return (parts||[]).reduce((s,p)=>s + (p && typeof p.marks==="number" ? p.marks : 0), 0);
  }
  ...
  window.QuestionBank = {
    ...
    /**
     * Generate one question instance.
     * @param {string} topicCode "N7" | "N8" | "N9"
     * @param {number} marksTotal 1..5
     * @param {string} paperMode "noncalc" | "calc"
     * @param {string|number} seedLike optional seed so a version can be locked
     */
    generate(topicCode, marksTotal, paperMode, seedLike){
      const rng = makeRng(seedLike);
      const q = buildQuestion(topicCode, Number(marksTotal), paperMode, rng);
      q.seed = rng.seed;
      q.topicName = __topicNameMap[topicCode] || topicCode;
      q.totalMarks = __sumMarks(q.parts);
      return q;
    }
  };
})();
